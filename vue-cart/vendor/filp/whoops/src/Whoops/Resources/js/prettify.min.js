...
;(function() {
    function O(a) {
        ...
        function j(d) {
            ...
            for (f = 0; f < d.length; ++f)
                (h = d[f]),
                    h[0] <= c[1] + 1
                        ? (c[1] = Math.max(c[1], h[1]))
                        : a.push((c = h))
            for (f = 0; f < a.length; ++f)
                (h = a[f]),
                    b.push(g(h[0])),
                    h[1] > h[0] &&
                        (h[1] + 1 > h[0] && b.push('-'), b.push(g(h[1])))
            b.push(']')
            return b.join('')
        }
        function t(d) {
    ...
    function G(a, i, g) {
        ...
        function t(a) {
            function i(a, b) {
                ...
                if (e) {
                    ...
                    e.appendChild(d)
                    for (var g = f; g; g = f)
                        (f = g.nextSibling), e.appendChild(g)
                }
                return d
            }
        ...
        for (
            var i = Math.max(0, (i - 1) | 0) || 0, o = 0, q = b.length;
            o < q;
            ++o
        )
            (m = b[o]),
    ...
    function n(a, i) {
        for (var g = i.length; --g >= 0; ) {
            ...
            A.hasOwnProperty(j)
                ? C.console &&
                  console.warn('cannot override language handler %s', j)
                : (A[j] = a)
        }
    }
    function F(a, i) {
    ...
    var S = (C.PR = {
        ...
        prettyPrint: (C.prettyPrint = function(a) {
            function i() {
                var u
                for (
                    var g = C.PR_SHOULD_USE_CONTINUATION
                        ? k.now() + 250
                        : Infinity;
                    m < j.length && k.now() < g;
                    m++
